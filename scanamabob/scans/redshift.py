                    ...
                    for permission in group["IpPermissions"]:
                        ...
                        for ip in permission["IpRanges"]:
                            ...
                            if cidr_ip == "0.0.0.0/0":
                                severity = "HIGH"
                                cluster["security_groups"][group_id].append(
                                    {"source": cidr_ip, "class": "Internet"}
                                )
                            elif context.is_aws_cidr(cidr_ip):
                                severity = "MEDIUM"
                                cluster["security_groups"][group_id].append(
                                    {"source": cidr_ip, "class": "AWS"}
                                )
    ...
    def run(self, context):
        ...
        for region in flagged:
            ...
            for group in flagged[region]:
                if (
                    group["group_name"] == "default.redshift-1.0"
                    and not group["in_use"]
                ):
                    default_group = group
                    break
            if default_group:
