...
from scanamabob.services.iam import (
    ...
    get_attached_iam_policy_documents,
)
from scanamabob.services.iam import (
    client,
    get_all_users,
...
class MfaScan(Scan):
    ...
    def run(self, context):
        ...
        if len(users_without_mfa):
            finding = Finding(
                context.state, self.title, "HIGH", users=users_without_mfa
            )
            return [finding]
        ...
        if len(users_over_max_password_age) > 0:
            return [
                Finding(
                    ...
                    self.title,
                    "MEDIUM",
                    users_over_max_password_age=users_over_max_password_age,
                )
            ]
        else:
            return []
                ...
                if type(poldoc) == list:
                    ...
                    for stmt in poldoc:
                        try:
                            if (
                                stmt["Effect"] == "Allow"
                                and stmt["NotAction"] is not None
                            ):
...
class PasswordPolicy(Scan):
    ...
    def run(self, context):
        ...
        try:
            ...
            return [
                Finding(context.state, "No AWS account-level password policy", "HIGH")
            ]
        res = list()
        if policy.minimum_password_length < 16:
            ...
            res.append(
                Finding(
                    ...
                    sev,
                    password_length=policy.minimum_password_length,
                )
            )
        complexity = (
